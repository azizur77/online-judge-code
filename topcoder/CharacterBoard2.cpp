#line 2 "CharacterBoard2.cpp"
#include <iostream>
#include <cstdio>
#include <cmath>
#include <vector>
#include <algorithm>
#include <utility>
#include <string>
#include <cstring>
#include <cassert>
#include <queue>
#include <set>
#include <numeric>
#include <stack>
using namespace std;
#define mp make_pair
#define INF (int)1e9
#define X first
#define Y second
#define REP(i, n) for(int i=0; i < n; i++)
#define FOR(i, a, b) for(int i=a; i < b; i++)
#define fill(a, x) memset(a, x, sizeof(a))
#define all(c) c.begin(), c.end()
#define sz(x)    ((int) x.size())
typedef vector<int> vi;
typedef pair<int, int> ii;
typedef long long ll;


class CharacterBoard2 {
	public:
	int countGenerators(vector <string> fragment, int W, int i0, int j0) {
		
	}
};




























// BEGIN CUT HERE
#include <ctime>
#include <cmath>
#include <string>
#include <vector>
#include <sstream>
#include <iostream>
#include <algorithm>
using namespace std;

int main(int argc, char* argv[])
{
	if (argc == 1) 
	{
		cout << "Testing CharacterBoard2 (1024.0 points)" << endl << endl;
		for (int i = 0; i < 20; i++)
		{
			ostringstream s; s << argv[0] << " " << i;
			int exitCode = system(s.str().c_str());
			if (exitCode)
				cout << "#" << i << ": Runtime Error" << endl;
		}
		int T = time(NULL)-1365730689;
		double PT = T/60.0, TT = 75.0;
		cout.setf(ios::fixed,ios::floatfield);
		cout.precision(2);
		cout << endl;
		cout << "Time  : " << T/60 << " minutes " << T%60 << " secs" << endl;
		cout << "Score : " << 1024.0*(.3+(.7*TT*TT)/(10.0*PT*PT+TT*TT)) << " points" << endl;
	}
	else
	{
		int _tc; istringstream(argv[1]) >> _tc;
		CharacterBoard2 _obj;
		int _expected, _received;
		time_t _start = clock();
		switch (_tc)
		{
			case 0:
			{
				string fragment[] = {"dea",
				                     "abc"};
				int W = 7;
				int i0 = 1;
				int j0 = 1;
				_expected = 1;
				_received = _obj.countGenerators(vector <string>(fragment, fragment+sizeof(fragment)/sizeof(string)), W, i0, j0); break;
			}
			case 1:
			{
				string fragment[] = {"xyxxy"};
				int W = 6;
				int i0 = 1;
				int j0 = 0;
				_expected = 28;
				_received = _obj.countGenerators(vector <string>(fragment, fragment+sizeof(fragment)/sizeof(string)), W, i0, j0); break;
			}
			case 2:
			{
				string fragment[] = {"gogogo",
				                     "jijiji",
				                     "rarara"};
				int W = 6;
				int i0 = 0;
				int j0 = 0;
				_expected = 0;
				_received = _obj.countGenerators(vector <string>(fragment, fragment+sizeof(fragment)/sizeof(string)), W, i0, j0); break;
			}
			case 3:
			{
				string fragment[] = {"abababacac",
				                     "aaacacacbb",
				                     "ccabababab"};
				int W = 882;
				int i0 = 10;
				int j0 = 62;
				_expected = 361706985;
				_received = _obj.countGenerators(vector <string>(fragment, fragment+sizeof(fragment)/sizeof(string)), W, i0, j0); break;
			}
			case 4:
			{
				string fragment[] = {"asjkffqw",
				                     "basjkffq",
				                     "qbasjkff",
				                     "qqbasjkf"};
				int W = 9031;
				int i0 = 9024;
				int j0 = 1959;
				_expected = 173947456;
				_received = _obj.countGenerators(vector <string>(fragment, fragment+sizeof(fragment)/sizeof(string)), W, i0, j0); break;
			}
			/*case 5:
			{
				string fragment[] = ;
				int W = ;
				int i0 = ;
				int j0 = ;
				_expected = ;
				_received = _obj.countGenerators(vector <string>(fragment, fragment+sizeof(fragment)/sizeof(string)), W, i0, j0); break;
			}*/
			/*case 6:
			{
				string fragment[] = ;
				int W = ;
				int i0 = ;
				int j0 = ;
				_expected = ;
				_received = _obj.countGenerators(vector <string>(fragment, fragment+sizeof(fragment)/sizeof(string)), W, i0, j0); break;
			}*/
			/*case 7:
			{
				string fragment[] = ;
				int W = ;
				int i0 = ;
				int j0 = ;
				_expected = ;
				_received = _obj.countGenerators(vector <string>(fragment, fragment+sizeof(fragment)/sizeof(string)), W, i0, j0); break;
			}*/
			default: return 0;
		}
		cout.setf(ios::fixed,ios::floatfield);
		cout.precision(2);
		double _elapsed = (double)(clock()-_start)/CLOCKS_PER_SEC;
		if (_received == _expected)
			cout << "#" << _tc << ": Passed (" << _elapsed << " secs)" << endl;
		else
		{
			cout << "#" << _tc << ": Failed (" << _elapsed << " secs)" << endl;
			cout << "           Expected: " << _expected << endl;
			cout << "           Received: " << _received << endl;
		}
	}
}

// END CUT HERE
